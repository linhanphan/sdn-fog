...
def myNetwork(sim, mode):
    ...
    info('*** Add links\n')
    h9s1 = {'bw': 1000}
    net.addLink(hosts[8], switches[0], cls=TCLink, **h9s1)
    ...
    h2s9 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[1], switches[8], cls=TCLink, **h2s9)
    #h4s9 = {'bw':100,'delay':'1ms'}
    #net.addLink(hosts[3], switches[8], cls=TCLink , **h4s9)
    h3s10 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[2], switches[9], cls=TCLink, **h3s10)
    #h6s10 = {'bw':100,'delay':'1ms'}
    #net.addLink(hosts[5], switches[9], cls=TCLink , **h6s10)
    h4s11 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[3], switches[10], cls=TCLink, **h4s11)
    #h8s11 = {'bw':100,'delay':'1ms'}
    #net.addLink(hosts[7], switches[10], cls=TCLink , **h8s11)
    h5s12 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[4], switches[11], cls=TCLink, **h5s12)
    #h10s12 = {'bw':100,'delay':'1ms'}
    #net.addLink(hosts[9], switches[11], cls=TCLink , **h10s12)
    h6s13 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[5], switches[12], cls=TCLink, **h6s13)
    #h12s13 = {'bw':100,'delay':'1ms'}
    #net.addLink(hosts[11], switches[12], cls=TCLink , **h12s13)
    h7s14 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[6], switches[13], cls=TCLink, **h7s14)
    #h14s14 = {'bw':100,'delay':'1ms'}
    #net.addLink(hosts[13], switches[13], cls=TCLink , **h14s14)
    h8s15 = {'bw': 100, 'delay': '1ms'}
    net.addLink(hosts[7], switches[14], cls=TCLink, **h8s15)
    ...
    for p in range(10001, 10009):
        hosts[-1].cmd('iperf -s -u -D -p ' + str(p))

    for i in range(1, 9):
        if True:
            hosts[i -
                  1].cmd('nohup python fog_agent7.py ' +
                         sim +
                         ' ' +
                         str(i) +
                         ' ' +
